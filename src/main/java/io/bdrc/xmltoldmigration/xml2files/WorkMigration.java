...
public class WorkMigration {
	...
	public static List<WorkModelInfo> MigrateWork(Document xmlDocument, Model m, Map<String,Model> itemModels) {
        ...
        if (isSeriesMember && !status.equals("withdrawn")) {
            ...
            if (!canonicalConceptualWork && !aWorkId.equals("WA0BC003"))
                mainA.addProperty(mA.createProperty(BDO, "language"), mA.createResource(BDR+"LangBo"));
            if (aWorkId.equals("WA0BC003"))
                mainA.addProperty(mA.createProperty(BDO, "language"), mA.createResource(BDR+"LangZh"));
            res.add(null);
            ...
            if (redirectionInstanceId == null && !status.equals("withdrawn") && !workId.startsWith("W1EAP") && !workId.startsWith("W1FPL") && (!workId.startsWith("W1FEMC") || CommonMigration.getConstraintWa('M'+workId, aWorkId) != null)) {
                ...
                if (otherAbstractRID == null || otherAbstractRID.equals(aWorkId)) {
                    ...
                    main.addProperty(m.createProperty(BDO, "instanceOf"), m.createResource(BDR+otherAbstractRID));
